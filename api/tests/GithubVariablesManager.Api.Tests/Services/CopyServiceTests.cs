...
public class CopyServiceTests
{
    private static CopyService CreateService(FakeHttpMessageHandler handler)
    {
        ...
        return new CopyService(new ItemMutationService(actionsRestClient, new SecretSealingService(), new CompositeVariableResolver(actionsRestClient), new CompositeManifestService(actionsRestClient)));
    }
