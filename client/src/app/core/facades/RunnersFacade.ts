 ...
 * runners for a repo scope, org runners for an org-only scope. Polled every 30s since
 * online/offline/busy status is live state, not something a manual mutation here ever changes —
 * a one-off fetch would go stale the moment a runner picks up a job.
 */
...
export class RunnersFacade {
  ...
  RunnersQuery(scope: () => DashboardScope | null) {
    return injectQuery(() => ({
      ...
      queryFn: () => {
        const s = scope()!;
        return s.repo ? this.runnersGateway.ListRepoRunners(s.org, s.repo) : this.runnersGateway.ListOrgRunners(s.org);
      },
