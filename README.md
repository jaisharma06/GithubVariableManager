...
- **Composite variables** — write a variable's value as a formula referencing other variables, Azure-App-Config style: `$(BASE_URL)/cdn`. As you type a `$(` reference, a dropdown suggests other variable names actually reachable from where you're editing (matching GitHub Actions' own override order, secrets excluded since they can never be referenced) — filtered as you keep typing, navigable with the arrow keys, and inserted with Enter/Tab or a click. Unlike a plain formula-only display trick, what's actually stored on GitHub is always a real, working value — resolved and written immediately, so it works in any real Actions workflow run with no extra manual step. The formula itself is tracked separately behind the scenes so it can always be recovered and recomputed; environment values win over repository, which win over organization, matching GitHub Actions' own override order. A reference to a variable that doesn't exist yet is shown as broken rather than blocked, so you can write formulas ahead of the variables they depend on; a formula that would reference itself (directly or through a chain) is rejected when you try to save it. If a variable a formula depends on changes later, the row is flagged stale — a routine, non-destructive **Sync** action (never automatic) recomputes it against the current values and updates it in place; the formula itself is never lost, so you can sync the same variable again anytime. A **Sync all** button appears above the list whenever the current scope has any composite variables at all, and re-syncs every one of them in a single action, reporting exactly which were updated, which were already current, and which failed. Variables only; a secret's value can never be a formula or be referenced by one, since GitHub never returns a secret's value to compute with in the first place. Behind the scenes, which variables are composite is tracked in one hidden-in-name-only variable per scope (`__GHVM_COMPOSITE_MANIFEST__`) — GitHub has no real "hidden" flag, so it's visible and editable in GitHub's own Settings → Actions → Variables page like any other variable. If it's ever hand-edited into something unparseable, you'll see a warning banner naming the affected scope (though, since that variable was the only record of which variables were composite, there's no way to know how many were affected — just that the scope's formula-tracking is broken and any composite variables there will need their formulas re-entered to restore Sync). If it's deleted outright, that's indistinguishable from "no composite variables were ever created here" — silent, not a bug this app can detect or warn about.
