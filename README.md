...
- **Composite variables** — write a variable's value as a formula referencing other variables, Azure-App-Config style: `$(BASE_URL)/cdn`. Resolved automatically every time the list loads (environment values win over repository, which win over organization — matching GitHub Actions' own override order), with no extra setup and nothing extra to store — the formula itself is the value GitHub keeps. A reference to a variable that doesn't exist yet is shown as broken rather than blocked, so you can write formulas ahead of the variables they depend on; a formula that would reference itself (directly or through a chain) is rejected when you try to save it. Since GitHub Actions runs have no idea what `$(...)` means, an "flatten to literal" action on any composite row overwrites it with today's resolved value — the only way a real workflow run ever sees a working value. Variables only; a secret's value can never be a formula or be referenced by one, since GitHub never returns a secret's value to compute with in the first place.
